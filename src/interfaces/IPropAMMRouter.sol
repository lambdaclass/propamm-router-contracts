...
interface IPropAMMRouter {
    ...
    /// legs' floors and the aggregate check all sit on one basis. The per-leg
    /// `Swapped` events report GROSS amounts; net user output is
    /// `SUM(Swapped.amountOut) − FrontendFeeCharged.feeAmount`.
