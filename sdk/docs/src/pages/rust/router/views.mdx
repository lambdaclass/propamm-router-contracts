...
```rust
let fallback: Address = router.fallback_swap_router().await?;
```
...
```rust
let fee: u32 = router.get_pair_fee(USDC, WETH).await?;
```
...
```rust
let venues: Vec<Address> = router.get_whitelisted_venues().await?;
```
