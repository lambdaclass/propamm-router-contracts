...
- `swapSplitV1(venues, probeHints, tokenIn, tokenOut, amountIn, amountOutMin, maxLegs, recipient, deadline)`: computes the split onchain — pulls funds, quotes each venue once (via the optional `IPropAMMPartialFill` extension, else a bounded two-point probe steered by the optional `probeHints`), ranks candidates by rate against a Uniswap V3 reference, and executes up to `maxLegs` propAMM legs plus one coalesced Uniswap remainder leg. Empty `venues` means the whole whitelist. Fee variants `swapMultiLegWithFeeV1` / `swapSplitWithFeeV1` skim the aggregate output (implementation-only, like the other `*WithFeeV1` entrypoints).
